 ...
 */
public class GraphMakeVisitor implements GJNoArguVisitor<Integer> {
    HashMap<Node, Set<String>> resultMap;
    HashMap<Node,InterferenceGraph> graphHashMap;
    HashMap<Node,Set<String>> paramMap;
    Set<String> tempHolder;
    InterferenceGraph graph;

    public HashMap<Node, InterferenceGraph> getGraphHashMap() {
        return graphHashMap;
    }

    public GraphMakeVisitor(HashMap<Node, Set<String>> r) {
        resultMap = r;
    ...
    public void removeParametersFromGraph(HashMap<Node,InterferenceGraph> graphHashMap){
        for(Node n:graphHashMap.keySet()){
            if(paramMap.containsKey(n)){
                for(String s:paramMap.get(n))
                    graphHashMap.get(n).deleteVertex(s);
            }
        }
    }
    public int registerLimit = 0;
    //
    ...
    public Integer visit(Goal n) {
        Integer _ret=null;
        String num = n.f0.tokenImage.substring(2,n.f0.tokenImage.length()-2);
        registerLimit = Integer.parseInt(num);
        n.f0.accept(this);
        n.f1.accept(this);
        n.f2.accept(this);
        return _ret;
...
//        }
    ...
    public Integer visit(MethodDeclaration n) {
        ...
        if (resultMap.containsKey(n)) {
           // System.out.println("Method Return : " + resultMap.get(n));
            graph.addEdgeBetweenAllOfThem(resultMap.get(n));
        } else {
            //System.err.println("LivenessQueryStatement missing: " + n);
        }
        graphHashMap.put(n,graph);
    ...
    public Integer visit(FormalParameter n) {
        Integer _ret=null;
        tempHolder.add(n.f1.f0.tokenImage);
        n.f0.accept(this);
        n.f1.accept(this);
        return _ret;
    }
